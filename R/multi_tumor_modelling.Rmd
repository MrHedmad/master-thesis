...
Testing the package `leaps` that was receommended for me from the professor.
...
sum_clin <- function(data, name){
  totn <- length(data$ID)
  n0mut <- length(data$ID[data$frequency == 0])
  perc0mut <- n0mut/totn*100
  sex <- table(data$gender)/totn*100
  cat(paste(
    "\nStatistics for clinical data - ", name, "\n",
    "Number of patients :          ", totn, "\n",
    "Nr. of pat. with no drivers : ", n0mut, "\n",
    "% of pat. with no drivers :   ", signif(perc0mut, digits=4), " %\n",
    "Mean Driver frequency :       ", signif(mean(data$frequency), digits=4), "\n",
    "Standard Deviation of Mean :  ", signif(sd(data$frequency), digits=4), "\n",
    "Mean Drivers w/o 0-patients : ", signif(mean(subset(data$frequency, data$frequency != 0)), digits=4), "\n",
    "% reported male:              ", signif(extract_safe(sex, "male"), digits=4), " %\n",
    sep=""
  ))
}

for (name in projects.names) {
  sum_clin(projects[[name]]$clinical, name)
}
```

First I want to look at the availabilities for the clinical data. I reuse the code I used a while ago:

```{r}
get_availability <- function(x, cutoff, cut_0_mut = FALSE){
  ...
  colnames(frame) <- "frac.availability"
  return(frame)
}

for (name in projects.names) {
  # I set an arbitrary cutoff of 0.01% in order to remove the cols with 0 availability
  projects[[name]]$availability <- get_availability(projects[[name]]$clinical, cutoff=0.01)
  print(projects[[name]]$availability)
}
```

First thing that we notice, is that there are different numbers of clinical variables for the different tumor types. Second, not all of them have 100% availability, this will need to add some nuances for the models.

Martina mixed gene level and mutation level in he rmodels, but I want to keep them separate. I'd need some reshaping to make the patients have a string of binary values to use as labels.
...
reshape_crystal <- function(project){
  ...
  gene_labels <- merge(gene_labels, project$clinical[,c("ID", "frequency", "passenger_freq")])
  return(gene_labels)
}
...
My question is if the data clusters based on the types of mutations present in the tumours.

There is the need to have some method of cluster quality analysis, but for a really, really, really rough estimate, since we would like to divide the clusters in high and low driver mutations, I just take the mean of the driver mutations in each of the clusters and normalize it by the average number of drivers of that tumor (to make it comparable across tumor types). I know this is bad, but I need some expert advice on what to choose as a quality metric.

```{r}
# This type of "center of mass" thingy is super skewed to outliers with very small
# frequency, so it's not good. I know that there are many more clustering quality functions
# but I need some sort of help choosing an appropriate one.
distance <- function(x, y, levels){
  ...
  y.0 <- mean(y[!levels])
  # Euclidian Distance
  return(sqrt(abs(x.1 - x.0)^2 + abs(y.1 - y.0)^2))
}
```

The plots for this are shown below in "testing" as it's probably a dumb thing to do.

## Testing

```{r}
# I just take one crystal
test <- projects$`TCGA-PAAD`$gene_labels
# And get rid of the hypermutators - this is completely arbitrary
test <- subset(test, test$passenger_freq < 300)

distance(test$passenger_freq, test$frequency, test$TP53)
distance(test$passenger_freq, test$frequency, test$BRCA1)

ggplot(test, aes(passenger_freq, frequency)) +
  geom_jitter(alpha=0.7, height=0.2, aes(colour = as.factor(TP53))) +
  scale_x_log10()

plot_distance <- function(labels, variable){
  cat(distance(labels$passenger_freq, test$frequency, labels[[variable]]))

  plot <- ggplot(labels, aes(passenger_freq, frequency)) +
...
result <- as.list(rep(NA, length(test) - 3))
names(result) <- names(test)[!(names(test) %in% c("ID", "frequency", "passenger_freq"))]

for (name in names(result)){
    result[[name]] <- distance(test$passenger_freq, test$frequency, test[[name]])
}

sort(unlist(result))

plot_distance(test, "KRAS")
plot_distance(test, "TP53")  #<-- This is pretty interesting (PAAD)
plot_distance(test, "TP53_R175H")
plot_distance(test, "CDKN2A")
plot_distance(test, "TP53_G245D")
```
