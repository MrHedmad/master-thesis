...
This file is based on `FinalModels` with the last tweaks for the thesis discussion. Mostly, additional factors have been added to the best models through forward model selection. I've clustered all of them in the end of each tumor's section.

To determine the best models fitted with glmulti, forward variable selection was performed following importance order starting from the model with the lowest AIC. If the variable taken in question was significant upon inclusion in the model, then it was kept and the next was added. If not, it was not added, and the process ended.

For the quasipoisson models, no forward variable selection was performed.
...
```{r}
projects$`TCGA-COAD`$clinical %>% colnames()
```
...
plot(models$coad$lowpass$negbi0, type = "s")
```
...
The best passenger model, with forward variable selection for adding any more information, is as follows:
...
projects$`TCGA-COAD`$clinical %>%
  filter(passenger.freq <= 50) %>%
  glm.nb(passenger.freq ~ age_at_diagnosis + driver.freq, data = .) ->
  models$coad$lowpass$best0

summary(models$coad$lowpass$best0)
```

For central passengers:

```{r}
projects$`TCGA-COAD`$clinical %>%
  filter(passenger.freq <= 300) %>%
  filter(passenger.freq < 50) %>%
  glm.nb(passenger.freq ~ driver.freq + age_at_diagnosis, data = .) ->
  models$coad$centralpass$best0

summary(models$coad$centralpass$best0)
...
projects$`TCGA-COAD`$clinical %>%
  filter(passenger.freq > 300) %>%
  glm.nb(passenger.freq ~ driver.freq + ajcc_pathologic_stage, data = .) ->
  models$coad$highpass$best0

summary(models$coad$highpass$best0)
```

In conclusion, there is no change.

### Driver

The best driver model is as follows:

```{r}
summary(models$coad$driver$qps2)
```
...
projects$`TCGA-BRCA`$clinical %>%
  filter(passenger.freq < quantile(.$passenger.freq, 0.95)) %>%
  glm.nb(
    passenger.freq ~ driver.freq + age_at_diagnosis + ajcc_pathologic_stage,
    data = .) ->
  models$brca$pass$best0

summary(models$brca$pass$best0)
```
...
projects$`TCGA-BRCA`$clinical %>%
  filter(passenger.freq < quantile(.$passenger.freq, 0.95)) %>%
  glm.nb(passenger.freq ~ age_at_diagnosis, data = .) ->
  models$brca$driver$best

summary(models$brca$driver$best)
```
...
```{r}
projects$`TCGA-LUAD`$clinical %>% colnames()
```
...
projects$`TCGA-LUAD`$clinical %>%
  filter(passenger.freq < 5000) %>%
  glm(
    driver.freq ~ age_at_diagnosis + ajcc_pathologic_stage,
    data = .,
    family = quasipoisson
    ) %>%
  {. ->> models$luad$driver$qps0} %>%
  diagnose(name = "Driver Model Quasi Poisson 0")
...
projects$`TCGA-LUAD`$clinical %>%
  filter(passenger.freq < 5000) %>%
  glm.nb(passenger.freq ~ driver.freq + age_at_diagnosis, data = .) ->
  models$luad$pass$best

summary(models$luad$pass$best)
```
...
```{r}
# No change
summary(models$luad$driver$qps1)
```
...
```{r}
summary(models$skcm$pass$importancenegbi)
```

### Driver

```{r}
summary(models$skcm$driver$importancenegbi)
```
...
```{r}
projects$`TCGA-GBM`$clinical %>% colnames()
```
...
projects$`TCGA-SKCM`$clinical %>%
  filter(passenger.freq < quantile(.$passenger.freq, 0.9) & driver.freq < 6) %>%
  glm(
    passenger.freq ~ age_at_diagnosis + prior_malignancy + driver.freq,
    data = .,
    family = quasipoisson
  ) %>%
...
projects$`TCGA-SKCM`$clinical %>%
  filter(passenger.freq < quantile(.$passenger.freq, 0.9) & driver.freq < 6) %>%
  glm(
    passenger.freq ~ prior_malignancy + driver.freq,
    data = .,
    family = quasipoisson
  ) %>%
...
```{r}
summary(best(models$gbm$passnolow$negbi0))
```

### Driver

```{r}
summary(models$gbm$driver$negbi0)
```
